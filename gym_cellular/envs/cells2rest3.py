...
class Cells2Rest3Env(gym.Env):
    ...
    def side_effects_func(self, state):
        ...
        for cell in range(n_cells):
            if state[cell] == 2:
                self.data['side_effects_incidence'] += 1.0/2.0

        side_effects=np.array(
                [['silent'] * n_cells] * n_cells,
                dtype='<U6',
            )
...
class PriorKnowledge:
    ...
    def __init__(self, **kwargs):
        ...
        self.state_space = [range(0,3)] * n_cells
        self.n_cells = len(self.state_space)
        self.action_space = [range(0,3)] * n_cells
        self.reward_range = (0, 1)
        self.initial_state = tuple([0] * n_cells)
        self.cell_classes = ['moderators']
        self.cell_labelling = [[0], []]
        ...
        self.n_actions = self.n_intracellular_actions ** n_cells
